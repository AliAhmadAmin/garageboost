...
"[project]/src/app/api/reminder-config/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function normalizeDays(values, fallback) {
    if (!Array.isArray(values)) return fallback;
    const parsed = values.map((value)=>Number(value)).filter((value)=>Number.isFinite(value) && value > 0 && value <= 365).map((value)=>Math.floor(value));
    const unique = Array.from(new Set(parsed)).sort((a, b)=>a - b);
    return unique.length > 0 ? unique : fallback;
}
function normalizeMonths(values, fallback) {
    if (!Array.isArray(values)) return fallback;
    const parsed = values.map((value)=>Number(value)).filter((value)=>Number.isFinite(value) && value > 0 && value <= 60).map((value)=>Math.floor(value));
    const unique = Array.from(new Set(parsed)).sort((a, b)=>a - b);
    return unique.length > 0 ? unique : fallback;
}
function parseCsv(csv) {
    return csv.split(",").map((value)=>Number(value.trim())).filter((value)=>Number.isFinite(value) && value > 0);
}
...
async function PUT(request) {
    ...
    const motDays = normalizeDays(body.mot, [
        30,
        14,
        7
    ]);
    const taxDays = normalizeDays(body.tax, [
        30,
        14,
        7
    ]);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c208c793._.js.map