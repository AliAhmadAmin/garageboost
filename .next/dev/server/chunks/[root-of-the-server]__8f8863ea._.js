...
"[project]/src/app/api/campaigns/queue-status/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    const jobs = await db.emailQueueJob.findMany({
        where: {
            campaignId: {
                in: campaignIds
            }
    ...
    for (const job of jobs){
        if (!job.campaignId || !byCampaign[job.campaignId]) continue;
        const status = String(job.status || "").toUpperCase();
        if (status === "PENDING") byCampaign[job.campaignId].pending += 1;
        else if (status === "PROCESSING") byCampaign[job.campaignId].processing += 1;
        else if (status === "RETRY") byCampaign[job.campaignId].retry += 1;
        else if (status === "SENT") byCampaign[job.campaignId].sent += 1;
        else if (status === "FAILED") byCampaign[job.campaignId].failed += 1;
    }
    for (const campaignId of campaignIds){
        byCampaign[campaignId].totalActive = byCampaign[campaignId].pending + byCampaign[campaignId].processing + byCampaign[campaignId].retry;
    }
    const totals = Object.values(byCampaign).reduce((acc, value)=>({
            pending: acc.pending + value.pending,
            processing: acc.processing + value.processing,
            retry: acc.retry + value.retry,
            sent: acc.sent + value.sent,
            failed: acc.failed + value.failed,
            totalActive: acc.totalActive + value.totalActive
        }), {
        pending: 0,
        processing: 0,
        retry: 0,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8f8863ea._.js.map